...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for (int ii = 0; ii < 12; ii++) {
        ...
        // Tube color
        float hue = hash_f(fi * 13.0) + tt * 0.05;
    ...
    float chromaAmt = corruption * 0.02;
    vec2 rOffset = vec2(chromaAmt, 0.0);
    vec2 bOffset = vec2(-chromaAmt, 0.0);
    float rShift = noise_f((uv + rOffset) * 50.0 + tt);
    float bShift = noise_f((uv + bOffset) * 50.0 + tt);
    col.r = mix(col.r, col.r + rShift * 0.2, corruption);
    col.b = mix(col.b, col.b + bShift * 0.2, corruption);
    ...
    col = mix(col, col.gbr * 1.2, blockGlitch * 0.5);
